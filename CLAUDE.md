...
| [DEC-027](docs/decisions/DEC-027.md) | 2026-08-28 | Alta | Modo 3 ("Ver as melhores oportunidades") passa a ranquear HEXÁGONOS sobre a base NACIONAL, com região virando filtro pós-ordenação; cascata do funil unificada numa redação só (as 2 divergências do ranking de UF viram parâmetro, payloads de `/api/estados`+`/api/uf`+`/api/municipio` idênticos); rota `GET /api/hexagonos`; READ-ONLY sobre o M1 | PENDENTE |
