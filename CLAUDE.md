...
| DEC-020 | 2026-07-23 | Estratégica | Escopo do corte do Streamlit pelo piloto web: Domínio fora (coberto pela Fase 4 do Mapa), Carteira vira Oportunidades Imobiliárias (placeholder), paridade só de Mapa + Visão Executiva + Viabilidade (BLK-WEB-11) — *renumerada de DEC-019 na reconciliação; corpo ainda em `docs/decisions/DEC-019.md` do `piloto-web`, a renomear* | APROVADA |
| [DEC-021](docs/decisions/DEC-021.md) | 2026-07-29 | Crítica | Raio do Relatório Pontual Censitário passa de 1,5 km para 1,0 km — análise, rótulo do método (`..._1km`, muda contrato da API) e metas absolutas dos Big Numbers reescaladas pela área; emenda a decisão-chave 5 da DEC-005 e o invariante do §3 | APROVADA |

<!-- RECONCILIACAO (merge piloto-web x main): os dois lados criaram uma DEC-019 DIFERENTE. Ficou com o numero 019 a da main (segundo dono de merge Critico), porque ela ja e' executada por `.github/workflows/guard.yml` (`DONOS = {"kastaldy", "vinhoabencoado"}`) e travada por `tests/contracts/test_grafo_ferramenta.py::test_dec_019_registrada_como_aprovada`. A DEC-019 do `piloto-web` (corte do Streamlit / BLK-WEB-11, APROVADA por Felipe em 2026-07-23) foi RENUMERADA para DEC-020 e entrou SEM link de propósito: `docs/decisions/DEC-020.md` ainda nao existe e `tests/unit/test_claude_md_size.py::test_indice_secao8_bate_com_docs_decisions` exige indice <-> arquivos casados 1:1. PENDENCIA: criar `docs/decisions/DEC-020.md` com o corpo que veio do `piloto-web` e trocar esta linha por `[DEC-020](docs/decisions/DEC-020.md)`. -->
