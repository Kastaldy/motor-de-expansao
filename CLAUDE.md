...
- Motor/UI censitario: `analisar_ponto_censitario_setores` cruza setor real x circulo 1.5 km em CRS metrico local; metodo `setor_censitario_intersecao_area_1p5km` (INTOCADO). `render_mapas_censitarios_combinados` (BLK-CENSO-01 / DEC-004) gera as 3 camadas (Densidade/Renda/Concorrentes) numa unica chamada, com **fundo de ruas por tiles online no relatorio** (CartoDB Positron No-Labels via `contextily`, extra `[basemap]`, import lazy + cache `data/cache/basemap_tiles/` + fallback offline gracioso — o dashboard interativo NAO depende de internet), composicao em `EPSG:3857`, **faixas absolutas fixas estilo GeoFusion** (`DENSIDADE_POP_BANDS`/`RENDA_PER_CAPITA_BANDS`/`RESIDUAL_SCORE_BANDS`, alpha 150) em vez de quartil, e **pins com logo** via `competitors._render_pin_tile` (Ultra=`"__ultra__"`, concorrente por `rede`, fallback de sigla); `render_mapa_censitario_estatico_png` segue como wrapper fino legado. `censo_report.py` gera CSV (inalterado) + PDF com **uma pagina de mapa por camada**; `render_relatorio_pontual_censitario` roda no expander do Mapa Territorial (sem dropdown de metrica), com lazy load/cache por `uf` + `cod_municipio` e mensagem clara quando a base geo nao existe. READ-ONLY sobre o M1: nao recalcula score nem altera artefatos.
...
- Provedor / licenca / atribuicao: **CartoDB Positron** — tiles (c) OpenStreetMap contributors, (c) CARTO; uso sob atribuicao. Atribuicao registrada no rodape do mapa/PDF ("(c) OpenStreetMap, (c) CARTO"); cache local minimiza requisicoes.
- Mitigacoes obrigatorias (parte da decisao):
  ...
  - (c) **Dependencia fora da carga do dashboard**: `contextily` em extra `[basemap]`, import lazy `try/except` dentro da funcao; a imagem de producao do dashboard interativo NAO depende de internet nem do extra.
