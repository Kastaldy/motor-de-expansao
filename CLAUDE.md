...
- Motor/UI censitario e Relatorio Pontual (mapas, camadas, slide-hero, PDF): contrato canonico em `docs/relatorio_pontual_censitario.md`; implementacao em `src/motor_expansao/dashboard/censo_map.py` e `censo_report.py`. Invariantes que valem sempre: o motor censitario (`setor_censitario_intersecao_area_1p5km`, `RAIO_CENSITARIO_DEFAULT_KM` = 1,5 km) e' INTOCADO e a camada e' READ-ONLY sobre o M1.
...
- Renda media domiciliar (READ-ONLY, visualizacao; concluido 2026-07-17): exibida no tooltip do hex, no grid 2x2 do Relatorio Pontual e no PDF da API/bot, sempre como `renda_pc_calibrada x moradores x uplift x FATOR_TEMPORAL_RENDA`. Formula, fontes de uplift e faixas em `docs/relatorio_pontual_censitario.md`.
...
- Dashboard tem **5 tabs**, nesta ordem: `Mapa` (default), `Executivo`, `Expansão de Domínio`, `Carteira e Plano`, `Viabilidade`. Fonte de verdade: `DASHBOARD_TAB_LABELS` em `dashboard/pages.py`, travada byte-a-byte por `tests/integration/test_streamlit_app.py`. (Ate 2026-07-27 esta linha dizia 4 tabs com os nomes antigos `Visao Executiva`/`Mapa Territorial`: a 5a aba entrou no BLK-DIM-12 / PR #34 e o rename no BLK-UI-01 / PR #36, nenhum dos dois tocou este arquivo.)
...
## 7. Onde aprofundar
- **PRIMEIRA PARADA — grafo de conhecimento (graphify):** antes de varrer arquivos para responder "como X funciona", "o que chama Y", "onde vive Z", **consultar o grafo**: `python -m graphify query "<pergunta>"`. Tambem `python -m graphify path "A" "B"` (caminho entre dois conceitos) e `... explain "<no>"`. **Usar `python -m graphify`, NAO o comando `graphify` nu** — o pacote instala `graphify.exe` em `<python>/Scripts/`, que normalmente **nao esta no PATH** (verificado em 2026-07-27). Sem o pacote: `pip install graphifyy`. Cobre 421 arquivos (290 de codigo por AST + 131 docs/contratos/DECs) em 7.560 nos e 15.351 arestas; benchmark do proprio graphify: **~58x menos tokens por consulta** que ler o corpus. Artefatos versionados: `graphify-out/graph.json` (consultavel) e `graphify-out/GRAPH_REPORT.md` (god nodes, coesao, trilha EXTRACTED/INFERRED/AMBIGUOUS).
- **Como o grafo se mantem atualizado (2 caminhos, NAO se substituem):**
  - **CODIGO — automatico.** Hooks `post-commit` + `post-checkout` (instalados em 2026-07-27 por `python -m graphify hook install`) disparam rebuild AST em segundo plano a cada commit/checkout. Sem LLM, sem custo de token, nao bloqueia o `git commit`. Log: `~/.cache/graphify-rebuild.log`. Pular pontualmente: `GRAPHIFY_SKIP_HOOK=1 git commit ...`. Um merge driver para `graphify-out/graph.json` fica registrado junto (evita conflito no artefato).
  - **DOCS/DECs/backlog — MANUAL.** O hook **ignora** mudanca em `.md`/`.yaml`: a camada semantica precisa de LLM. Depois de mexer em `docs/`, `tasks/`, `CLAUDE.md` ou nas DECs, rodar **numa sessao Claude**: `python -m graphify . --update` (so re-extrai o que mudou; despacha sub-agentes so' para os arquivos alterados).
  - **Os hooks vivem em `.git/hooks/` e NAO sao versionados.** Cada clone/maquina precisa rodar `python -m graphify hook install` uma vez (`... hook status` confere). O container do loop e o CI nao tem o pacote — la o grafo nao atualiza.
- **LIMITES do grafo, ler antes de confiar:** (a) entre um commit de doc e o `--update` manual, a camada semantica esta DESATUALIZADA — se a resposta depender de doc recem-alterado, ler o arquivo; (b) a consulta trunca por orcamento de tokens (usar `--budget` quando vier cortado); (c) `context/handoff/` (566 logs) e as imagens ficaram **fora** do grafo de proposito; (d) e' INDICE de navegacao, **nao** fonte canonica — parametro, formula e guardrail valem pelo §3/§8 e pelos contratos em `docs/`.
