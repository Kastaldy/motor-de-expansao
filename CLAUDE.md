...
## 5. Ciclo ativo
- Ciclo atual do `PRD.md`: penetracao, TAM/SAM e oferta efetiva por hexagono.
- Objetivo: usar dados reais das unidades Ultra (`data/ultra/dados_academias.xlsx` + `data/ultra/Ultra.csv`) para calibrar penetracao, padroes de desempenho e potencial residual de mercado.
- Entregas esperadas do ciclo: `unidades_ultra_performance.parquet`, `unidades_ultra_performance_hex.parquet`, reports de validacao GeoFusion/hex e penetracao, colunas novas de TAM/SAM/oferta residual na camada de mercado e correcao do tooltip do hex pesquisado.
- Bloco 1 fechado em 2026-05-15: `jobs/pipelines/normalizar_unidades_ultra.py` gera `data/staging/unidades_ultra_performance.parquet` com 54 unidades de performance, 53 com coordenada/`hex_id_res7`; `status_match_coord` audita casamentos e `CAMPO LIMPO - SP` permanece `sem_coord`.
- Bloco 2 fechado em 2026-05-15: `jobs/pipelines/calcular_penetracao_ultra_hex.py` gera `data/staging/unidades_ultra_performance_hex.parquet` com 54 unidades; 49 casam com a camada de mercado, fontes de populacao: 28 `censo_2022_hex`, 21 `m1_municipal_proxy`, 4 `hex_nao_encontrado`, 1 `sem_hex_id_res7`; metricas: `pop_hex_base`, `fonte_pop_hex_base`, penetracoes, receita por habitante, `alunos_por_m2` e rankings.
- Bloco 3 fechado em 2026-05-15: `jobs/pipelines/comparar_geofusion_vs_hex.py` gera `data/reports/validacao_geofusion_vs_hex.md`; densidade e o KPI principal, diferenca bruta de populacao e diagnostico secundario.
- Bloco 4 fechado em 2026-05-15: `jobs/pipelines/validar_penetracao_ultra_hex.py` gera `data/reports/validacao_penetracao_ultra_hex.md`; classifica top/bottom por tercis em 6 lentes, calcula correlacoes Pearson/Spearman e lista outliers IQR sem remove-los. Cautela canonica: penetracao e receita por habitante usam `pop_hex_base` no denominador, entao correlacoes com populacao/densidade do hex sao diagnosticas, nao causais.
- Bloco 5 fechado e revisado em 2026-05-15: `jobs/pipelines/calcular_colunas_mercado.py` adiciona sizing absoluto de TAM/SAM fitness sem alterar o M1; `taxa_fitness_mercado_calibrada` calculada em runtime como mediana de `(n_academias_2km * 2000) / pop_hex_base` em todos os hexes com academias — resultado **20%** com 28 redes mapeadas (era 4.5% fixo com apenas 3 redes); `oferta_consumida_total_estimada = oferta_consumida_mercado_estimada + oferta_consumida_ultra_estimada` descontada do SAM para gerar `oferta_efetiva_disponivel`; concorrentes: auto-discovery de todos os `unidades_*.csv` de `concorrentes/` (3.179 unidades validas); `n_redes_mapeadas` e contagem dinamica (atualmente 28).
- Bloco 7 fechado em 2026-05-15: `jobs/pipelines/enriquecer_outputs_residual_mercado.py` propaga TAM/SAM fitness, consumo estimado, Ultra real, `oferta_efetiva_disponivel`, `score_oportunidade_residual`, `share_ultra_estimado_hex` e quartil auxiliar para hibrido, carteira e plano; dashboard mostra residual em tooltips, legenda, filtro de quartil e ordenacao opcional da Carteira, mantendo M1 como ordenacao padrao.
- Bloco 8 fechado em 2026-05-15: bug de inflate do SAM corrigido em `calcular_colunas_mercado.py`; `pop_hex_base` agora usa `pop_total_setor_2022` para 1.302.296 hexes (antes apenas 850 via gate `flag_censo_elegivel`); fallback `populacao_proxy / total_hex_municipio` para 230.349 hexes; labels `censo_2022_setor` e `m1_municipal_proxy_per_hex`; `oferta_efetiva_disponivel` total: 3.17M alunos (era 1.27B); dashboard sinaliza `(est. proxy)` no tooltip SAM; `fonte_pop_hex_base` adicionada as map_columns; parquets propagados; 270/270 testes passando.
- Handoff/deploy Streamlit anterior esta fechado; artefatos minimos do dashboard seguem em `data/outputs/`: `hexagonos_brasil_dashboard.parquet`, `oportunidades_expansao_hibrido.parquet`, `carteira_expansao_acionavel.parquet`, `plano_expansao_curto_prazo.parquet`.
- API/FastAPI, PostGIS, Prefect, pipelines pesados, M2/M3, pesquisas e Power BI continuam fora do deploy inicial, concentrados em `fora_primeira_fase/`.
...
- Visual pop-cut e destaque de busca (Bloco 3, 2026-05-14; tooltip completo em 2026-05-15): hex com `flag_pop_min_5k=False` recebe cor cinza `[120,120,140,70]` nos mapas (via `_apply_pop_cut_colors`); legenda `render_pop_cut_legend` exibe chip "Descartado <5k hab"; `build_map_figure`/`build_hybrid_map_figure` aceitam `search_hex_id: str | None` e adicionam `H3HexagonLayer` amarelo de destaque acima das demais camadas — aparece mesmo se o hex estiver fora dos filtros ou descartado; quando o `hex_id` existe na base, o hover do destaque reutiliza o tooltip completo do hex normal; `streamlit_app.py` deriva `search_hex_id` via `lookup_hex_by_coord`.
