...
def _normalizar_serie_disponivel(serie: pd.Series, considerar_zero_como_ausente: bool = True) -> pd.Series:
...
def calcular_hex_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["concorrencia_norm"] = normalizar_serie(1 / (df["n_academias_osm"].fillna(0) + 1))
    df["vitalidade_norm"]   = df["score_vitalidade"].fillna(50.0)
    df["hex_score"] = (
