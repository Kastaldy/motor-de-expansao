...
Ordem de fechamento (execução): **(6.0) housekeeping de concluídos via helper → (6.a) commit por path → (6.b) merge pelo humano → (6.c) dry-run autônomo → (6.d) rollback se preciso**.

5. **(6.0) Housekeeping de concluídos (roda ANTES do commit 6.a).** Se — e somente se — o bloco do ciclo APROVADO existe como bloco completo em `tasks/backlog.md` (ciclos com tarefa ad-hoc, fora do backlog, PULAM este passo: o resumo já entra em `completed.md` via Passo 6.2):
   - **Mover via helper versionado:** rode `python scripts/housekeeping_move_block.py <BLK-ID> --date <AAAA-MM-DD>`. O helper move o bloco íntegro (fatia literal byte-idêntica, preservando CRLF) de `tasks/backlog.md` para `tasks/completed.md` e substitui o bloco no backlog por stub de 1 linha (`- BLK-XXX (concluído AAAA-MM-DD) — ver tasks/completed.md`). NÃO editar `backlog.md`/`completed.md` à mão; o helper é a única via (byte-identity garantida e testada em `tests/unit/test_housekeeping_helper.py`).
   - **Ciclo ad-hoc:** se o bloco não existe no backlog, o helper sai com **código 3** (`EXIT_AD_HOC`, mensagem `SKIP:` no stderr) → trate como no-op (sem erro): o resumo do ciclo já vai a `completed.md` via Passo 6.2. Qualquer outro código ≠ 0 é falha real (ver abaixo).
...
8. **(6.c) Dry-run autônomo pós-merge (gate — só para ciclos que alteram a orquestração).**
   ...
   - **Cobertura do 6.0 no dry-run:** como o dry-run testa a orquestração, ele também exercita o 6.0 — adiciona um bloco DUMMY mínimo a `tasks/backlog.md` NA BRANCH do dry-run, fecha o ciclo dummy (6.0 chama o helper → stub no backlog + bloco em `completed.md`) e confirma via `--check`. Como a branch do dry-run NUNCA é mergeada, o bloco dummy não polui `main`; abandono não-destrutivo via `git switch <branch-anterior>` ao final.
...
- **Housekeeping de concluídos via helper (6.0):** ao fechar ciclo APROVADO cujo bloco veio do `tasks/backlog.md`, mover o bloco SEMPRE via `scripts/housekeeping_move_block.py` (nunca à mão) — move byte-idêntico + stub de 1 linha. Ciclo ad-hoc (bloco fora do backlog) → `BlockNotFound` = no-op. O QA exige `--check` verde antes do veredito final (ver `prompts/qa_analyzer.md`).
