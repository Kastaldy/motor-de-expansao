...
### 5. Estado do repositório na branch — o rebuild em background SUJOU o repo DURANTE esta sessão
...
- **Às 09:44:59 o `git status --porcelain` estava VAZIO. Às 09:53 estava assim:**
...
```
  ...
  Causa atribuída por timeline + log (`~/.cache/graphify-rebuild.log`), **não** por suposição: o
  rebuild disparado pelo hook `post-checkout` na criação da branch rodava em background e **terminou
  às 09:45:22** (mtime idêntico nos três arquivos). O log fecha com
  `[graphify watch] Rebuilt: 13599 nodes, 21063 edges, 741 communities`.

**Isso não é churn inofensivo — o rebuild automático produz um grafo DIFERENTE do curado:**

| | Grafo versionado (BLK-GRAPH-01) | Rebuild automático do hook |
|---|---|---|
| Nós / arestas | 7.560 / 15.351 (`CLAUDE.md` §7) | **13.599 / 21.063** |
| `graph.json` | 10.157.850 bytes | **15.299.593 bytes** (+50%) |
| `context/handoff/` | **fora do corpus, de propósito** (§7, limite "c") | **dentro** — o log mostra os `.md` de `context/handoff/` sendo processados |
| `.graphify_labels.json` | rótulos de comunidade **curados à mão** (exceção explícita no `.gitignore`, linhas 165–168) | **sobrescrito**, +744 linhas |

  O rebuild ainda fez backup do grafo curado em `graphify-out/2026-07-28/` (ignorado pelo padrão de
  data do `.gitignore`) e emitiu `warning: 249 source file(s) produced zero nodes`.

**Consequências para o Planner (isto é escopo de A3, não fofoca):**
1. Versionar esses hooks em `.githooks/` **propaga esse comportamento para todo clone**. O hook que
   vamos abençoar é o mesmo que viola o recorte documentado do corpus e atropela a curadoria de
   rótulos. Decidir conscientemente: versionar como está + documentar, ajustar o hook, ou restringir
   o que ele reconstrói.
2. O Builder vai encontrar o repo sujo **sem ter feito nada** — e o hook re-suja a cada commit.
   `git checkout -- graphify-out/` antes de commitar, e **commit por path** (nunca `git add -A`).
3. Bônus de tranquilidade: o cap de tamanho do servidor MCP é **512 MiB**
   (`GRAPHIFY_MAX_GRAPH_BYTES`, `security.py` linha 32) — nem 10 MB nem 15 MB chegam perto. O
   inchaço não quebra o A1.
...
8. `git status --porcelain` conferido antes do commit e **`graphify-out/` restaurado**
   (`git checkout -- graphify-out/`): os três artefatos aparecem modificados sozinhos por causa do
   rebuild em background do hook (ver Evidência 5). Commit **por path**, nunca `git add -A`.
   O diff final do PR não pode conter `graphify-out/*`.
...
- **R10 (descoberto durante este BO, o mais insidioso) — o hook de rebuild suja o repo sozinho e o
  faz com um grafo DIFERENTE do curado.** O rebuild em background terminou no meio desta sessão e
  deixou `graph.json`, `GRAPH_REPORT.md` e `.graphify_labels.json` modificados (13.599 nós vs 7.560
  versionados; `context/handoff/` entrou no corpus contra o recorte da §7; rótulos curados
  sobrescritos). Dois riscos concretos: **(a)** o Builder commita 5 MB de artefato regenerado sem
  querer e afoga o `claude-review` (mesmo modo de falha do R5/R7); **(b)** versionar esse hook em
  `.githooks/` propaga o comportamento para todo mundo. O Planner precisa decidir explicitamente o
  que fazer com isso dentro do A3 — não é aceitável abençoar o hook sem registrar a divergência.
