...
ao setor IBGE) onde ela cai, aos scores existentes (M1, censitário, residual, domínio) e aos desfechos
observados (alunos / faturamento / sinal Wellhub-Gympass). Insumo do backtest (BLK-SCORE-02).
Read-only sobre o M1: só leitura e join, nenhuma escrita em artefato M1.

**Achados de mapeamento (todos verificados em disco — números reais):**

1. **Conversão lat/lng→H3 é inline, não há módulo `geo/`/`data_sources/`/geocoder.**
   Padrão canônico: `h3.latlng_to_cell(lat, lng, 7)` (fallback `h3.geo_to_h3`), ver
...
2. **Unidades Ultra (performance + hex + scores já resolvidos):**
   ...
   `agregadores`, `faturamento`, `ticket_medio_aluno`, `metragem`, `alunos_por_m2`,
   `penetracao_ultra_*`. **Cobertura do join: 49/54** com `hex_id` + `score_priorizacao` +
   `score_expansao_hibrido` (5 sem coordenada casada). NÃO tem coluna `rede`, NÃO tem `cod_municipio`,
   NÃO tem `score_setor_2022_calibrado`/`score_oportunidade_residual`/`score_dominio_hibrido` nem
...
3. **Concorrentes mapeados:** `data/staging/concorrentes_mapeados.parquet` (11 colunas; gitignored).
   ...
   **ATENÇÃO — Skyfit NÃO está neste parquet (0 linhas de `rede="skyfit"`).** A fonte
   `concorrentes/unidades_skyfit.csv` existe (91 linhas) mas o snapshot normalizado não a incluiu
   (provável: arquivo não casou o glob `unidades_*.csv` no momento da geração, ou foi gerado antes do
   add). Existem também `concorrentes/SkyFit_unidades_geocodificado.csv` e
   `concorrentes/Sky_Fit_Unidades_geocoded.xlsx`. **Decisão necessária:** o Planner precisa decidir se
   re-roda `normalizar_concorrentes.py` (que regrava `concorrentes_mapeados.parquet` — artefato de
   STAGING, NÃO oficial M1) para incluir Skyfit, ou se lê o CSV Skyfit direto no script de montagem e
   resolve o hex via `h3.latlng_to_cell`. Sem isso, Skyfit fica de fora do dataset.
...
4. **Fonte dos scores por hex (chave de join = `hex_id` = H3 res.7):**
   ...
   - **4º score (domínio):** `score_dominio_hibrido` existe SÓ em
     `data/outputs/plano_expansao_dominio.parquet` (**500 linhas**, por `hex_id`). Logo a cobertura do
     score de domínio é restrita aos 500 hexes do plano — a maioria das unidades ficará com domínio
     NULO. Tratar como esperado e marcar (não é falha de join).
   - **Setor IBGE:** este parquet resolve `cod_municipio`/`nome_municipio`, não `cod_setor`. Resolução
     de setor real (geometria) viria de
     `data/outputs/setores_censitarios_2022_geo/uf=XX/cod_municipio=NNNNNNN/part-000.parquet` via
     `resolve_cod_municipio_from_geo_dir` (dashboard/data.py, linha ~88) — pesado e provavelmente fora
     do necessário. RECOMENDAÇÃO: para BLK-SCORE-01 anexar `cod_municipio` + `score_setor_2022_calibrado`
     do parquet de mercado e deixar o setor censitário detalhado como opcional/decisão do gate.

5. **Rótulos de desfecho dos concorrentes (parte frágil) — planilhas em `data/validacao/` (gitignored):**
   - `Sky Fit dados.xlsx` → sheet `Sell Out` (347 linhas; header na **linha 2**, não na 1):
     colunas `UNIDADE | ALUNOS | TICKET MEDIO | FATURAMENTO | WELLHUB | FUNCIONAMENTO`. **Tem coluna
     WELLHUB** (sinal de agregador para Skyfit!) e `FUNCIONAMENTO` (possível proxy de
     maturação/horário — confirmar semântica). O Planner deve tratar o offset de cabeçalho (skiprows).
   - `academias_engenharia_do_corpo.xlsx` → sheets ricos: `Academias` (62 linhas:
     ...
     Alunos Totais | Observações`), `Base_Calculo` (alunos/m²), `Estacionamento`, `Fontes`. Ou seja,
     EngCorpo TEM alunos absolutos (`Alunos Totais`, `Total Alunos Ativos`) e Gympass — melhor que o
     README sugeria (não é só alunos/m²).
   - Join unidade-mapeada ↔ planilha é por NOME (não há id/coord nas planilhas), maior fonte de erro.

6. **Sinal Wellhub/Totalpass:** Ultra → `alunos_gympass`+`alunos_totalpass`; Skyfit → coluna `WELLHUB`
   na planilha; EngCorpo → `Total Alunos Gympass`. Disponível para as TRÊS redes (Gympass=Wellhub),
   mas com nomes/origens diferentes — unificar como coluna proxy de demanda por agregador.

7. **`data_abertura`/maturação:** NÃO ENCONTRADO como data explícita em nenhuma fonte. Único candidato
   indireto: `FUNCIONAMENTO` na planilha Skyfit (semântica a confirmar). Sem fonte canônica de data de
   abertura para Ultra e EngCorpo → flag de maturação fica majoritariamente indisponível. Decisão do gate.
## Objetivo
...
- Obter unidades EngCorpo de `concorrentes_mapeados.parquet` (`rede=="engenharia_do_corpo"`,
...
- QUALQUER escrita em artefato M1 ou alteração de score/fórmula/pesos/carteira/plano/domínio.
  ...
  `data/staging/concorrentes_mapeados.parquet`, que é staging paralelo, NÃO artefato oficial M1 — ok se
  o Planner optar por isso, mas é decisão a registrar.)
...
- Planilhas `data/validacao/Sky Fit dados.xlsx` (sheet `Sell Out`, header na linha 2) e
...
- `concorrentes/unidades_skyfit.csv` (91 linhas, `nome_unidade,latitude,longitude,data_coleta`) caso o
  Planner opte por ler Skyfit direto.
- **NÃO ENCONTRADO (sinalizar):** fonte de `data_abertura`/maturação; `score_dominio_hibrido` por hex
  fora dos 500 do plano; `cod_setor_2022` no parquet de mercado; módulo geo central.
...
- Cada unidade tem `hex_id_res7` resolvido e os scores disponíveis anexados
  ...
  `score_dominio_hibrido`); ausência de score por falta de cobertura (domínio 500 hexes; unidades sem
  coordenada) fica MARCADA em coluna de status, não descartada.
...
- Auditoria de rótulo: relatório curto de nulos/outliers em alunos, com nota de confiabilidade
  (Skyfit `ALUNOS` medido vs estimado; EngCorpo `Alunos Totais` vs `Alunos/m²`).
- Sanidade do join: nº de unidades de entrada == nº com score anexado, OU diferença explicada.
- `pytest -q tests/unit/test_validation_dataset.py` verde; `pytest -q` sem regressão (baseline
...
- **Skyfit ausente do staging (alto):** sem ação explícita, uma das três redes-alvo fica fora. Decidir
  via de inclusão.
- **Join de rótulo por nome (alto):** planilhas só têm nome; casamento fuzzy com unidades mapeadas.
  Cardinalidades não batem: Skyfit ~91 (planilha) vs 91 (csv) mas 0 no staging; EngCorpo 63 mapeadas
  vs 62 na planilha `Academias`. Exige reconciliação e relatório de não-casados.
- **Maturação sem fonte (alto):** sem `data_abertura`; só `FUNCIONAMENTO` (Skyfit, semântica incerta).
- **Cobertura parcial do score de domínio (médio):** só 500 hexes no plano; maioria fica nulo.
- **Cobertura parcial de hex (médio):** 5/54 Ultra sem hex; unidades em hex fora do mercado mapeado
  ficam sem scores censitário/residual.
- **Header não-padrão na planilha Skyfit (médio):** header na linha 2; ler com skiprows ou tratamento.
...
1. **Skyfit:** aprovar a via de inclusão — (a) re-rodar `normalizar_concorrentes.py` para regravar o
   staging com Skyfit, ou (b) o script ler `concorrentes/unidades_skyfit.csv` e resolver hex via H3.
2. **Maturação:** sem `data_abertura`. Aceitar `maturacao_indisponivel` nesta entrega (e tratar
   maturação em BLK-SCORE-02 quando houver fonte) OU fornecer planilha de datas / aprovar
   `FUNCIONAMENTO` como proxy.
3. **Rótulo canônico de "alunos":** definir a coluna única (Ultra `alunos_total`/`ativos_pag`; Skyfit
   `ALUNOS`; EngCorpo `Alunos Totais`/`Total Alunos Ativos`) e como sinalizar medido vs estimado.
4. **Setor censitário:** confirmar que basta `cod_municipio`+`score_setor_2022_calibrado` (do mercado)
   e que NÃO é preciso resolver geometria de setor via `setores_censitarios_2022_geo`.
5. **Score de domínio com cobertura parcial:** confirmar que anexar só nos 500 hexes do plano e marcar
   nulo no resto é aceitável.
6. **Versionamento:** `data/analysis/dataset_validacao.parquet` está gitignored (verificado) — manter
   fora do git (contém dados sensíveis de concorrentes). Confirmar.
