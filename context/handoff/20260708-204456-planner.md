...
Block Orchestrator + Planner (combinados)
...
- Bloco de LEITURA/DOCUMENTAÇÃO pura. Zero alteração de código, `config.py`,
  `pipelines/m1/`, score, pesos ou artefatos oficiais.
- Único arquivo de produto a criar: `docs/arquitetura_app_atual.md`.
- Loop-safe (§6.1): sem VPS, sem rede, consome só leitura do repo.
- Pesos `renda=0.40`/`pop=0.60`, `score_priorizacao`, `hex_score_estrutural`,
  carteira, plano e artefatos oficiais INALTERADOS.

## Artefato a criar
`docs/arquitetura_app_atual.md` — ESCRITO ABAIXO (já materializado neste ciclo).

## Instrução ao Builder
O documento completo está em `## Conteúdo completo do artefato` abaixo E já foi
escrito em `docs/arquitetura_app_atual.md`. Basta confirmar que o arquivo existe
com esse conteúdo (byte-a-byte com o bloco abaixo) e commitá-lo por path. Nenhuma
outra alteração de código é necessária. Se o arquivo já estiver presente e correto,
apenas verifique e siga para o commit + housekeeping das tasks.

## Fontes lidas (evidência)
- `CLAUDE.md` §2/§4/§5 (lazy por UF, render lazy de abas, fonte de mapa enxuta,
  API, censo, guardrails).
- `tasks/current_task.md` (BLK-REV-02).
- `streamlit_app.py` (entry, loaders cacheados, `main()`, dispatch de abas).
- `dashboard/data.py` (carga/merge/derivação/agregadores — lido completo).
- `dashboard/components.py` (builders de mapa, downsample, caps, pydeck — via inventário).
- `dashboard/pages.py` (abas, `render_tab_selector`, sidebar/busca, fragment `@st.fragment`).
- `dashboard/constants.py` (caps, colunas, UFs, COLOR_MODES/OVERLAYS — lido completo).
- `dashboard/censo_report.py` e `dashboard/relatorio_municipal.py` (estrutura dos PDFs).
- `src/motor_expansao/api/main.py` + routes/service (endpoints).
- `data/analysis/perf_baseline_app_2026.md` (timing BLK-REV-01).
- Sizes de disco via `du -sh` em `data/outputs/` e `data/staging/`.

## Correções factuais aplicadas na escrita (vs. leitura inicial dos sub-agentes)
- `render_mapa_pydeck_fragment` É decorado com `@st.fragment` (pages.py:4099) —
  confirmado por grep direto (um relatório inicial dizia que não era).
- `MAP_POINT_LIMIT = 35000` e `MAP_POINT_LIMIT_LARGE = 18000` (valores canônicos de
  `constants.py`, não os 7000/34999 citados por um sub-agente).
- Loaders pesados usam `@st.cache_resource`; catálogos/leves usam `@st.cache_data`.
  Nenhum TTL configurado. Verificado em `streamlit_app.py`.

## Paths do ciclo
- `docs/arquitetura_app_atual.md` (novo, a commitar)
- `tasks/backlog.md` (housekeeping: mover/atualizar status de BLK-REV-02)
- `tasks/completed.md` (housekeeping: registrar conclusão)
- `tasks/current_task.md` (status)
- `context/handoff.md` (este) + `context/handoff/20260708-204456-planner.md`

## Guardrails verificados
- §5 READ-ONLY M1: nenhuma escrita em `src/`, `config.py`, `pipelines/m1/`, score
  ou artefatos oficiais. Só cria `docs/arquitetura_app_atual.md`.
- §6.1 loop-safe: sem VPS, sem rede, sem PII, consome só `data/` local (leitura).
- DEC-001 intacta (pesos/fórmula do M1 inalterados).
- Documento é descritivo do estado atual; não propõe mudança de código.
