...
_COLS_NOMEADAS = [
    "nome",
    # `[DEC-063]` De qual app veio a academia. O artefato SEMPRE teve a coluna; o piloto e' que a
    ...
    "fonte",
    # `[DEC-063 / fatia 2]` QUAIS apps a listam (`alvos_ma_nomeados_v8`). Mesma armadilha da linha
    ...
    itens = [
        {
            ...
            # `[DEC-063]` O app que revelou a academia -- decide a arte do pino no mapa.
            "fonte": _texto(getattr(t, "fonte", None)),
            # `[DEC-063 / fatia 2]` Com virgula (`"totalpass,wellhub"`), o pino sai no terceiro
...
def _base_calibracao() -> tuple[pd.DataFrame | None, str]:
    """Base de comparaveis DESTE deploy — adaptador sobre a funcao unica.

    O piloto sabe onde o staging mora (`STAGING_DIR`); a funcao movida nao, de proposito.
    """
    return base_calibracao(STAGING_DIR)


def _payload_viabilidade(body: ViabilidadeIn) -> dict[str, Any]:
    """`viabilidade_payload_v1` para o PILOTO — injeta o que e deste deploy.

    Adaptador FINO por decisao: a montagem e uma so (FIN-VIAB-01) e vive em
    `motor_expansao.dimensionamento.payload_viabilidade`. Aqui entram apenas as duas
    coisas que dependem de ONDE o codigo roda: o diretorio de staging e a malha do
    catchment. Os tres consumidores do piloto (`/api/viabilidade`,
    `/api/relatorio/pontual`, `/api/simulador/xlsx`) continuam chamando ESTA funcao.

    `setores_df` NAO e opcional na pratica: sem ele o catchment nao roda e
    `flag_zona_morta` sai `None` em 100% das chamadas, apagando em silencio o aviso da
    tela e o gate E4 da Conclusao (DEC-042).
    """
...
def _rede_concorrencia_nova() -> dict[str, Any]:
    ...
    `[DEC-063]` O TotalPass segue FORA, mas a RAZÃO mudou — e a antiga virou falsa. Ela dizia
    ...
    positivo que a DEC-061 existe para barrar. Presença e churn estão fora do escopo da DEC-063
...
def _viabilidade_pdf_payload(body: ViabilidadeIn) -> dict[str, Any] | None:
    """Ponte do PDF para o piloto — injeta staging e catchment na funcao unica.

    Mesmo papel do `_payload_viabilidade`: a montagem vive no `src/`, e daqui saem so' as
    duas coisas que dependem de onde o processo roda.
    """
    return montar_payload_para_pdf(
        body,
        staging_dir=STAGING_DIR,
        setores_df=_setores_para_catchment(body.lat, body.lng),
    )
