...
def _saida_valida() -> pd.DataFrame:
    ...
    `[DEC-066]` Até a DEC-066 os três regimes eram `{s1,s3,s4}`, o ramp-up `{s1}` e o vazio. Com o
...
def test_pesos_do_d4_seguem_congelados_e_somam_um() -> None:
    ...
    # `[DEC-066]` O `s1` entrou em `SINAIS_INATIVOS`, e a linha acima é o que prova que isso NÃO
...
def test_s6_entra_no_regime_quando_o_insumo_vem(serie_madura_s3_s4: list[pd.DataFrame]) -> None:
    ...
    # `[DEC-066]` 3, não 4: o `s1` saiu da conta (`SINAIS_INATIVOS`).
...
def test_regime_so_s1_NAO_OCORRE_MAIS(serie_rampup: list[pd.DataFrame]) -> None:
    """`[DEC-066]` O regime `{s1}` deixou de existir, e a consequência é ausência — não zero.

    Até a DEC-066 este teste provava o ramp-up: S3/S4 fora, S1 com peso `1,00`, score `{0, 50}`.
...
def test_regime_de_um_sinal_agora_e_alcancavel_pela_rota_ordinaria(
    ...
    """`[DEC-066]` O regime de UM sinal deixou de exigir frame forjado.
...
def test_regime_s3_s4(serie_madura_s3_s4: list[pd.DataFrame]) -> None:
    ...
    `[DEC-066]` Chamava-se `test_regime_s1_s3_s4`. O Plano B tinha três sinais porque o `s1`
...
def test_regime_so_s3() -> None:
    ...
    `[DEC-066]` **Até 2026-09-17 esta docstring dizia que o regime era "inalcançável pelo caminho
...
def test_ordenavel_nao_separa_regimes_de_tamanho_diferente() -> None:
    ...
    # `[DEC-066]` `{1, 2}`: o regime completo virou `{s3, s4}` quando o `s1` saiu da conta. A
...
def test_v1_por_mapa_categorico() -> None:
    ...
    out = calcular_score_vulnerabilidade(churn=churn, presenca=presenca)
    # `[DEC-066]` O mapa continua EXISTINDO e continua sendo o que governa o `v1` no dia em que o
...
def test_nenhum_percentil_toca_v1_nem_v3() -> None:
    ...
    # `[DEC-066]` O `v1` saiu da conta e é nulo dos dois lados — comparar `float(nan)` provaria
...
def test_rampup_sem_pressao_fica_SEM_NOTA(serie_rampup: list[pd.DataFrame]) -> None:
    """`[DEC-066]` O ramp-up produzia `{0, 50}` — os dois valores do `v1`. Agora produz ausência.
...
def test_rampup_coluna_ordenavel_e_toda_nula(serie_rampup: list[pd.DataFrame]) -> None:
    ...
    `[DEC-066]` A segunda asserção INVERTEU: antes o score existia (era `{0, 50}`) e só o
...
def test_pressao_tira_o_rampup_do_regime_provisorio(serie_rampup: list[pd.DataFrame]) -> None:
    ...
    # A ordenação só faz sentido se houver o que ordenar, e `[DEC-066]` mudou O QUE HÁ antes do
    # S6 — não a emenda. Até a DEC-066 a régua sem S6 era CATEGÓRICA (o `v1` só assumia
...
def test_score_zero_no_rampup_nao_significa_nao_vulneravel(
    ...
    out = calcular_score_vulnerabilidade(churn=churn, presenca=presenca)
    linha = _linha_de(out, "k_tp")
    # `[DEC-066]` O `0` que este teste explicava NÃO PODE MAIS OCORRER: ele vinha do `v1` valendo
...
def test_join_da_mesma_serie_nao_tem_left_only(serie_madura_s3_s4: list[pd.DataFrame]) -> None:
    ...
    churn, presenca = _insumos(serie_madura_s3_s4)
    out = calcular_score_vulnerabilidade(churn=churn, presenca=presenca)
    # `[DEC-066]` A prova de que os dois frames casam é a coluna de AUDITORIA, não o `v1`: com o
...
def test_assert_schema_rejeita_flag_provisorio_incoerente() -> None:
    # `[DEC-066]` A linha forjada mudou de `k_so_s1` para `k_sem_sinal`, e a razão é a fixture: com
...
def test_assert_schema_rejeita_versao_inesperada() -> None:
    # O valor invalido e' DERIVADO da constante, nunca um literal de versao. Ate' a DEC-066 este
...
def test_status_churn_e_fato_propagado_sem_peso() -> None:
    ...
    # `[DEC-066]` Com pressão: sem ela as duas linhas ficariam SEM NOTA, e `null == null` é falso
