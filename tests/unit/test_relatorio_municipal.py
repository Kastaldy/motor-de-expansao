...
def test_mapas_municipio_offline_sem_rede():
    ...
    assert set(mapas) == {"resumo", "score", "residual", "dominio"}
...
def test_pdf_municipal_fallback_sem_dominio_pagina_6():
    """Sem dominio -> Paginas 5-6 em modo simplificado, sem excecao; nota de bairros presente."""
    df = _sample_df()
    ...
    assert b"/Count 8" in pdf_bytes
    # Nota D9 (bairros indisponiveis).
    assert b"Bairros indisponiveis na base atual" in pdf_bytes
    # Mensagem de zonas indisponiveis na pagina de dominio.
    assert b"Zonas de dominio nao disponiveis" in pdf_bytes
