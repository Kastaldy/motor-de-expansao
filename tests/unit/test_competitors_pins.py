...
def test_a_arte_do_totalpass_e_CONVERSAO_FIEL_e_nao_copia_de_bytes():
    """`[DEC-065]` O invariante do TotalPass e' DIFERENTE do WellHub, e de proposito.
...
def test_ambos_COM_arte_compoe_as_duas_logos(tmp_path):
    """`[DEC-065 / fatia 2]` M2: placa branca com as DUAS logos, uma por metade.
...
def test_ambos_SEM_arte_cai_na_placa_bipartida_B5(tmp_path):
    """`[DEC-065 / fatia 2]` A degradacao escolhida — e a que MAIS importa.
...
def test_a_arte_de_AMBOS_e_a_composicao_EXATA_das_duas_atuais():
    """`[DEC-065 / fatia 2]` O asset composto nao pode envelhecer em silencio.
...
def test_a_chave_sai_da_coluna_fontes_da_academia():
    """`[DEC-065 / fatia 2]` O coracao da fatia: virgula -> `__ambos__`.
...
def test_os_dois_apps_se_distinguem_por_COR_e_nao_so_por_arte():
    """`[DEC-065]` Sem PNG nenhum o tile cai na placa SOLIDA da marca.
    ...
    # `[DEC-065 / fatia 2]` TRES chaves, DUAS artes — e a assimetria e' o DESENHO, nao esquecimento:
