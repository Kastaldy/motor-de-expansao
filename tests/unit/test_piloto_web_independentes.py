...
def test_o_pin_declara_de_QUAL_app_a_academia_veio(com_independentes: Path) -> None:
    """`[DEC-065]` A arte do pino sai da `fonte` — então a `fonte` tem de CHEGAR ao item.
    ...
    ficaria vermelha** — que é exatamente o defeito que a DEC-065 corrigiu.
    """
    misto = _nomeadas()
...
def test_o_pin_declara_quando_a_academia_esta_nos_DOIS_apps(com_independentes: Path) -> None:
    """`[DEC-065 / fatia 2]` A coluna `fontes_da_academia` tem de CHEGAR ao item.
