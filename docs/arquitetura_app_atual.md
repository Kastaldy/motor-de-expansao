...
- `gerar_pdf_relatorio_pontual_censitario(result, mapas=None, *, residual=None, ultra_dir=None, solicitante=None, rotulo=None) -> bytes` — **5 páginas**: Capa → Mapas de calor (tira 1×3 Densidade/Renda/Score, cada PNG embutido) → Concorrentes → Big Numbers (grid 4×2 de 8 métricas) → Realização/Crédito. Recebe os PNGs de mapa já compostos (por `mapas`), embute e adiciona marca d'água; não faz fetch de tiles aqui (os PNGs vêm de `censo_map`).
...
- `gerar_pdf_relatorio_municipal(municipio_result, mapas=None, *, ultra_dir=None, solicitante=None, versao=None) -> bytes` — **~8-9 páginas** (Capa, Visão Geral/Cobertura, Resumo, Score Censitário, Residual Fitness, Expansão de Domínio, Bairros por Zona, Síntese, Espaço e academias).
