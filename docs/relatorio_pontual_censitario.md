...
Regras implementadas:
- **8 camadas canonicas, geradas numa unica chamada** (`CAMADAS_CENSITARIAS` = `densidade`, `renda`, `score`, `renda_domiciliar`, `socioeconomia`, `residual`, `concorrentes`, `entorno`) e distribuidas por 4 paginas do PDF — **NAO** mais uma pagina por camada (o BLK-RELPON-01 consolidou os choropleths): "Imagem do Entorno" (grid 1x1, camada `entorno`), "Socioeconomia e Residual Fitness" (grid 2x1, `socioeconomia`+`residual`), "Mapas de calor" (grid 2x2, `densidade`/`renda`/`score`/`renda_domiciliar`) e "Concorrentes" (camada `concorrentes`). A UI nao tem dropdown que esconda camadas. Densidade/Renda/**Score**/Renda domiciliar/**Socioeconomia** compartilham basemap, bbox, projecao, pins e choropleth (varia so o fill dos setores + legenda/titulo); a camada **Score censitario** usa o choropleth de `score_setor_2022_calibrado` (`_color_for_score`) COM legenda de faixas 0-100 (`_score_legend_entries()`); a camada **Concorrentes e Ultra e SO de pins** (sem choropleth — BLK-CENSO-03): basemap + pins de concorrentes/Ultra + ponto central + legenda de pins; as camadas **`residual`** (5 km) e **`entorno`** (escala de quadra) tem frame/bbox proprios e NAO desenham pins.
...
- `gerar_pdf_relatorio_pontual_censitario(result, mapas=None, *, residual=None, perfil_bairro=None, ultra_dir=None, solicitante=None, rotulo=None, fotos=None, info_imovel=None, viabilidade=None)`: retorna bytes PDF em memoria com a **estrutura de 8 paginas** (template Ultra; **BLK-RELPON-01** consolidou os 3 choropleths em 1 slide; **BLK-RELPON-07** inseriu "Perfil do Bairro/Distrito"; **BLK-RELPON-10** inseriu o slide-hero "Socioeconomia e Residual Fitness"; **BLK-RELPON-11** inseriu "Imagem do Entorno"), nesta ordem (a variante `gerar_pdf_relatorio_pontual_classico` tem a MESMA sequencia de 8 paginas e a mesma assinatura, mais o parametro `now` para data determinista em teste):
  ...
  3. **Socioeconomia e Residual Fitness** (BLK-RELPON-10, gate Vinicius 2026-07-21) — slide-hero com 2 mapas lado a lado (grid 2x1): `socioeconomia` = o MESMO choropleth de `score_setor_2022_calibrado` no raio de 1,5 km (o `score` PERMANECE tambem no grid 2x2 — S1=A do gate; e um 2o render, nao uma migracao) e `residual` = choropleth de `oferta_efetiva_disponivel` (em ALUNOS) por hexagono H3 res-7 num raio de EXIBICAO `RAIO_RESIDUAL_DISPLAY_KM = 5.0` km (disco `h3.grid_disk` k=5, clip ao frame), com regua absoluta de 6 faixas `OFERTA_DISPONIVEL_ALUNOS_BANDS` ancorada em 2.500 alunos = 1 unidade. A camada `residual` e CONDICIONAL: sem `hexes_df`/sem hex desenhavel a celula cai no fallback textual (offline-safe).
