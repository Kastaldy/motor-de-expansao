...
> Versao: 2026-07-22 (atualizada no BLK-RELPON-11; PDF de 8 paginas).
...
- Conjunto de mapas estaticos PNG (ate 8 camadas numa unica geracao): as camadas de 1.5 km trazem setores, circulo do raio, ponto central e pins opcionais de concorrentes/Ultra; `residual` (5 km) e `entorno` (escala de quadra) tem frame proprio e NAO desenham pins -- ver §6.
...
Implementacao: `src/motor_expansao/dashboard/censo_map.py`. Criado no ciclo BLK-CENSO-01 (DEC-004) e ampliado depois por BLK-CENSO-03/-FU5, BLK-RELPON-06, renda domiciliar (PR #126), BLK-RELPON-10/-FU1 (`socioeconomia`/`residual`) e BLK-RELPON-11 (`entorno`).

Funcao principal: `render_mapas_censitarios_combinados(lat, lng, setores_df, *, raio_km=1.5, competitors_df=None, ultra_df=None, width=1000, height=760, basemap=True, logos_dir=None, ultra_logo_dir=None, street_ceil=None, street_gain=None, street_cap=None, choropleth_alpha=None, hexes_df=None) -> dict[str, bytes]`. Retorna ate 8 camadas numa unica geracao — as 6 INCONDICIONAIS `{"densidade", "renda", "score", "renda_domiciliar", "concorrentes", "entorno"}` mais as CONDICIONAIS `"socioeconomia"` e `"residual"` (BLK-RELPON-13: ambas so entram quando `hexes_df` foi passado, tem a coluna do dado — `score_setor_2022_calibrado` / `oferta_efetiva_disponivel` — E ha pelo menos 1 hex desenhavel; sem elas a celula do slide cai no fallback textual). Chaves canonicas e ordem em `CAMADAS_CENSITARIAS` (`score` restaurado no BLK-CENSO-03-FU5; `renda_domiciliar` em 2026-07-17, PR #126; `socioeconomia`/`residual` no BLK-RELPON-10, `socioeconomia` migrada para hexagono a 5 km no BLK-RELPON-13; `entorno` no BLK-RELPON-11). O legado `render_mapa_censitario_estatico_png(...)` continua como wrapper fino que devolve UMA camada pelo antigo `metric_column` (`score_setor_2022_calibrado` -> `score`) e NAO conhece `socioeconomia`/`residual`/`entorno` — nao serve para montar o PDF.
...
Regras implementadas:
- **8 camadas canonicas, geradas numa unica chamada** (`CAMADAS_CENSITARIAS` = `densidade`, `renda`, `score`, `renda_domiciliar`, `socioeconomia`, `residual`, `concorrentes`, `entorno`) e distribuidas por 4 paginas do PDF — **NAO** mais uma pagina por camada (o BLK-RELPON-01 consolidou os choropleths): "Imagem do Entorno" (grid 1x1, camada `entorno`), "Socioeconomia e Residual Fitness" (grid 2x1, `socioeconomia`+`residual`), "Mapas de calor" (grid 2x2, `densidade`/`renda`/`score`/`renda_domiciliar`) e "Concorrentes" (camada `concorrentes`). A UI nao tem dropdown que esconda camadas. Densidade/Renda/**Score**/Renda domiciliar compartilham basemap, bbox, projecao, pins e choropleth (varia so o fill dos setores + legenda/titulo); a camada **Score censitario** usa o choropleth de `score_setor_2022_calibrado` (`_color_for_score`) COM legenda de faixas 0-100 (`_score_legend_entries()`); a camada **Concorrentes e Ultra e SO de pins** (sem choropleth — BLK-CENSO-03): basemap + pins de concorrentes/Ultra + ponto central + legenda de pins; as camadas **`socioeconomia`** (BLK-RELPON-13: `score_setor_2022_calibrado` por hexagono H3 a 5 km, mesma paleta 0-100 do `score`), **`residual`** (5 km) e **`entorno`** (escala de quadra) tem frame/bbox proprios e NAO desenham pins.
...
- Exibe circulo de 1.5 km (referencia do raio), ponto central como **pin vermelho**, legenda por camada, escala visual, coordenada central, metodologia e atribuicao `(c) OpenStreetMap, (c) CARTO` no rodape. **Excecoes das camadas com frame proprio:** `residual` desenha o circulo do raio de EXIBICAO de 5 km e NAO desenha pins nem a legenda de pins (`pins=[]`, `ultra_pins=[]`, `mostrar_legenda_pins=False` — BLK-RELPON-10-FU1: a 5 km os logos tapavam o choropleth); `entorno` nao desenha circulo algum (`circle_3857=None`) nem pins, mantem so o pin vermelho central, usa o prefixo `"Escala de quadra"` no rodape (parametro `rotulo_escala` de `_render_camada`) em vez de "Raio X km" e sobrescreve o zoom LOCALMENTE (`zoom_bump=-1` -> z18, escolha do gate visual) -- a constante global `_BASEMAP_ZOOM_BUMP` continua `1` e serve os frames de 1,5 km e 5 km.
...
- **Refinamento visual BLK-EST-02 (tema "Ultra Clean / GeoFusion", gate Felipe 2026-06-11):**
  - **Titulos curtos (D6=A):** os 4 mapas de entao (hoje sao 8 camadas, todas com titulo curto; nas de escala diferente de 1,5 km o raio entra no proprio titulo) perderam o prefixo repetitivo `"Relatorio Pontual Censitario - "` (a camada ja aparece na faixa de titulo do PDF) -> `"Densidade populacional"`, `"Renda per capita"`, `"Score censitario"`, `"Concorrentes e Ultra"`. O **subtitulo tecnico** (coordenada/raio/setores) e mantido para auditoria inline (o PNG tambem serve a UI). Tamanho do titulo aumentado no BLK-RELPON-06 (ver abaixo).
...
- `gerar_pdf_relatorio_pontual_censitario(result, mapas=None, *, residual=None, perfil_bairro=None, ultra_dir=None, solicitante=None, rotulo=None, fotos=None, info_imovel=None, viabilidade=None)`: retorna bytes PDF em memoria com a **estrutura de 8 paginas** (template Ultra; **BLK-RELPON-01** consolidou os 3 choropleths em 1 slide; **BLK-RELPON-07** inseriu "Perfil do Bairro/Distrito"; **BLK-RELPON-10** inseriu o slide-hero "Socioeconomia e Residual Fitness"; **BLK-RELPON-11** inseriu "Imagem do Entorno"), nesta ordem (a variante `gerar_pdf_relatorio_pontual_classico` tem a MESMA sequencia de 8 paginas e a mesma assinatura, mais o parametro `now` para data determinista em teste):
  ...
  2. **Imagem do Entorno** (BLK-RELPON-11) — UM mapa de quadra ocupando a pagina (grid 1x1 empacotado, camada `entorno`): basemap CartoDB Voyager resolvido em z18 (`zoom_bump=-1` LOCAL desta camada; a constante global `_BASEMAP_ZOOM_BUMP` continua 1), SEM choropleth, SEM pins de concorrentes/Ultra e SEM circulo de raio — so o pin vermelho central. Raio de EXIBICAO `RAIO_ENTORNO_DISPLAY_KM = 0.14` km (lado menor do frame = 2 x 140 m x (1 + `_MAP_FRAME_MARGIN`=0.08) = 302,4 m) e rodape com o prefixo `"Escala de quadra"` no lugar de "Raio X km". Camada INCONDICIONAL (devolve sempre bytes; sem tiles cai no canvas claro).
  3. **Socioeconomia e Residual Fitness** (BLK-RELPON-10, gate Vinicius 2026-07-21; **socioeconomia revista no BLK-RELPON-13, gate Vinicius 2026-07-23**) — slide-hero com 2 mapas lado a lado (grid 2x1): `socioeconomia` = choropleth de `score_setor_2022_calibrado` por **hexagono H3 res-7** num raio de EXIBICAO `RAIO_RESIDUAL_DISPLAY_KM = 5.0` km (disco `h3.grid_disk` k=5, clip ao frame), com a **MESMA metrica e a MESMA paleta** (`score_band_to_color`, 0-100) do `score` do grid 2x2 — o que muda e' a **geometria/escala** (hex a 5 km, nao setor a 1,5 km), reusando o caminho de render do `residual`; o `score` PERMANECE no grid 2x2 (S1=A do gate). `residual` = choropleth de `oferta_efetiva_disponivel` (em ALUNOS) por hexagono H3 res-7 no MESMO raio de 5 km, com regua absoluta de 6 faixas `OFERTA_DISPONIVEL_ALUNOS_BANDS` ancorada em 2.500 alunos = 1 unidade. **Ambas** as camadas sao CONDICIONAIS: sem `hexes_df`, sem a coluna do dado (`score_setor_2022_calibrado` / `oferta_efetiva_disponivel`) ou sem hex desenhavel, a celula cai no fallback textual (offline-safe). As 2 imagens do slide sao reduzidas por `_HERO_MAP_SCALE` (fator de escala calibrado no gate visual). SEM pins nas duas camadas.
  4. **Mapas de calor** — os 4 choropleths (`densidade`, `renda`, `score`, `renda_domiciliar`) num **grid 2x2** (BLK-RELPON-01 consolidou a tira 1x3; a 4a camada entrou com a renda media domiciliar, PR #126), sobre fundo claro `#F8F8F8` + faixa de titulo. Cada PNG (do BLK-CENSO-01) e embutido **separadamente** (nao pre-composto), com sua **legenda embutida** (as escalas sao distintas: densidade hab/km², renda R$/pessoa, score 0-100 — restaurado COM legenda no BLK-CENSO-03-FU5 —, renda domiciliar R$/domicilio). Camada ausente -> fallback textual "Mapa indisponivel para esta camada." naquela celula (offline-safe).
  5. **Concorrentes** — mapa de concorrentes (mapa `concorrentes` SO de pins: basemap + pins de concorrentes/Ultra + ponto central, SEM choropleth) a esquerda + lista textual das redes no raio a direita (coluna `rede`/`nome_unidade`, NUNCA dados de pessoas).
  ...
  7. **Big Numbers** — grid 4x2 das 8 metricas (ver abaixo).
  8. **Realizacao/Credito** — fundo turquesa solido, texto fixo "Relatorio gerado pelo Motor de Expansao - Ultra Academia", atribuicao de tiles `(c) OpenStreetMap, (c) CARTO` e nota READ-ONLY. SEM PII.
  ...
  - **Paginas OPCIONAIS (nao alteram a ordem base das 8):** `fotos` -> `_fotos_imovel_page` e `info_imovel` -> `_info_imovel_page` entram logo APOS a Capa (antes de "Imagem do Entorno"); `viabilidade` -> `_viabilidade_page` entra entre Big Numbers e Realizacao e vale 1 pagina ("Viabilidade - Números") ou 2 (mais "Viabilidade - Projeção financeira") quando `viabilidade["graficos"]` traz PNGs. Teto com todas as opcionais = **12 paginas** (`/Count 12`).
...
**Insercao BLK-RELPON-10 (gate Vinicius 2026-07-21):** o slide-hero "Socioeconomia e Residual Fitness" foi inserido ANTES de "Mapas de calor", levando o PDF de **6->7 paginas**, `/Count 6`->`/Count 7`, `PDF_SECTION_HEADERS` de 6->7 strings. Feito nas duas variantes. 2 mapas lado a lado via `_draw_maps_grid` generalizado com `cols`/`rows` KEYWORD-ONLY (default 2x2 preservado -> o caminho de "Mapas de calor" ficou byte-identico): `socioeconomia` = MESMO choropleth de `score_setor_2022_calibrado` no raio de 1,5 km (o `score` PERMANECE no grid 2x2, S1=A do gate) e `residual` = `oferta_efetiva_disponivel` em ALUNOS por hexagono H3 res-7 no raio de EXIBICAO de 5 km (S2=B do gate; emenda a DEC-011, que limitava o residual a NUMERO nos Big Numbers). O slide usa `_tema_bicolor(0)` (magenta) e NAO inverte a cor das paginas ja existentes (ordinais decrescentes a partir de 0 para paginas inseridas antes das 4 paginas de conteudo historicas). **BLK-RELPON-10-FU1 (2026-07-22):** a camada Residual deixou de desenhar pins de concorrentes/Ultra e a legenda de pins (a 5 km a area e ~11x a de 1,5 km e os logos tapavam o choropleth). Motor censitario (`setor_censitario_intersecao_area_1p5km`, raio 1,5 km) INTOCADO.

**Insercao BLK-RELPON-11 (caminho A1, gate Vinicius 2026-07-22):** a pagina "Imagem do Entorno" foi inserida entre a Capa e o slide-hero, levando o PDF de **7->8 paginas** (Capa->Imagem do Entorno->Socioeconomia e Residual Fitness->Mapas de calor->Concorrentes->Perfil do Bairro/Distrito->Big Numbers->Realizacao), `/Count 7`->`/Count 8`, `PDF_SECTION_HEADERS` de 7->8 strings. Feito nas duas variantes (`_entorno_page`/`_classico_entorno_page`). Grid 1x1 empacotado com a camada `entorno`; tema `_tema_bicolor(-1)` = turquesa (`-1 % 2 == 1` em Python), sem inverter as paginas existentes. Motor censitario e artefatos oficiais INTOCADOS.

**Correcao BLK-RELPON-13 (gate visual Vinicius 2026-07-23):** o painel `socioeconomia` do slide-hero deixou de ser o choropleth de setores a 1,5 km e passou a ser `score_setor_2022_calibrado` por **hexagono H3 res-7 num raio de EXIBICAO de 5 km** (disco `h3.grid_disk` k=5, clip ao frame) — a MESMA metrica e a MESMA paleta (`score_band_to_color`, 0-100) do `score` do grid 2x2, mudando so a geometria/escala para casar com o `residual` ao lado. Implementado GENERALIZANDO o caminho do `residual` (`_render_camada_residual_hex`/`_hex_polygons_3857`/`_residual_hex_central` ganharam parametros keyword-only DEFAULT-PRESERVING para `value_col`/titulo/legenda/cor/valor central -> o `residual` fica byte-a-byte igual). Consequencia: `socioeconomia` virou **CONDICIONAL** como o `residual` (sem `hexes_df`/coluna/hex desenhavel -> chave ausente + fallback textual; SEM pins), e a faixa superior passou a usar `_legenda_valor_hex` ("Score no hexagono: NN"). As 2 imagens do slide sao reduzidas por `_HERO_MAP_SCALE` (constante de RENDER de `censo_report.py`, **0.85**, calibrada por Vinicius no gate visual de 2026-07-24 -- partiu de 0.92; so as paginas socioeconomia+residual a aplicam via `packed_scale` do `_draw_maps_grid`/`_map_grid_cells_packed`). `/Count 8`, as 8 chaves de `CAMADAS_CENSITARIAS` e as demais 7 camadas seguem byte-identicas. Feito nas duas variantes. READ-ONLY sobre o M1: `score_setor_2022_calibrado` e apenas LIDO do `hexes_df`; motor censitario (`setor_censitario_intersecao_area_1p5km`, raio 1,5 km) INTOCADO.

**Refinamento visual BLK-EST-02 (tema "Ultra Clean / GeoFusion", gate Felipe 2026-06-11):** so estilo/geometria/strings — o grid **4x2 de 8 metricas** READ-ONLY, `set_compression(False)` e `pdf_version="1.4"` permanecem INTOCADOS (a contagem de paginas/ordem foi atualizada depois por BLK-RELPON-01, -07, -10 e -11, acima; hoje sao 8 paginas).
...
Validacao minima: `tests/unit/test_relatorio_pontual_censitario_export.py` cobre bytes CSV/PDF, os **8** headers de secao, `/Count 8`, as imagens de mapa embutidas no PDF (**entorno + socioeconomia/residual + os 4 choropleths do grid 2x2 "Mapas de calor" + a de Concorrentes so-pins**, separadamente e nao pre-compostas, via `/Subtype /Image >= 5`), Big Numbers com/sem residual ("n/d"), fallback offline sem assets, anti-PII, atribuicao de tiles, retrocompat de `bytes` unico e helper de download; os testes `test_pdf_concorrentes_contagem_total_e_mais_n_quando_excede_10` e `test_pdf_concorrentes_sem_contagem_quando_ate_10` cobrem o cabecalho/contagem D4=B do BLK-EST-02 (sem PII). **BLK-RELPON-01** adiciona `test_slide_unico_quatro_mapas_sem_sobreposicao` (bounding boxes das celulas do grid, recente + classico), `test_slide_unico_count_8_e_titulo_mapas_de_calor`, `test_slide_unico_offline_safe_por_camada` e `test_slide_unico_quatro_imagens_embutidas`. **BLK-RELPON-05** nao muda logica em `censo_report.py` (a faixa ja vem "assada" nos PNGs recebidos como `mapas: dict[str, bytes]`); `test_pdf_estrutura_inalterada_com_faixa_valor_ponto_blk_relpon_05` reforca que `/Count 8`, contagem de imagens e headers permanecem identicos. **BLK-RELPON-07** adiciona `test_perfil_bairro_page_presente_com_4_metricas_recente`, `test_perfil_bairro_page_nd_quando_perfil_bairro_none` e `test_classico_perfil_bairro_page_presente_e_nd` (nova pagina "Perfil do Bairro/Distrito", com/sem `perfil_bairro`), alem de atualizar os 13 asserts `/Count 5`->`/Count 6` e as 2 contagens de marca d'agua `>= 5`->`>= 6` nos testes existentes (5->6 paginas); em `tests/unit/test_relatorio_pontual_censitario_motor.py`, `test_lookup_bairro_ponto_quando_setor_tem_bairro`, `test_lookup_distrito_ponto_fallback_quando_bairro_ausente` e os testes de `agregar_perfil_bairro_distrito` (agregacao, fallback, exclusao simetrica de renda, "n/d" sem identificador/DataFrame vazio) cobrem o motor novo. **BLK-RELPON-10** adiciona `test_slide_hero_presente_nas_2_variantes_com_8_paginas`, `test_slide_hero_offline_safe_sem_camada_residual`, `test_tema_bicolor_ordinal_zero_e_magenta_sem_mexer_nos_existentes`, `test_map_layer_titles_inclui_as_chaves_novas_senao_os_pngs_sumiriam` e `test_grid_de_mapas_aceita_2x1_e_1x1_e_mantem_o_default_2x2`; **BLK-RELPON-11** adiciona `test_pagina_entorno_presente_nas_2_variantes_com_8_paginas` e `test_pagina_entorno_offline_safe_sem_a_camada`. O teto de 12 paginas com as opcionais e travado em `tests/unit/test_relatorio_pontual_viabilidade.py` e `tests/unit/test_relatorio_pontual_orquestracao.py` (`/Count 12`).

### Implementacao do Bloco 6

O fluxo visual foi integrado ao dashboard em `render_relatorio_pontual_censitario`, dentro do expander `Relatorio Pontual Censitario` na aba `Mapa Territorial`. Ele reutiliza a coordenada ativa do clique no mapa ou da busca da sidebar, mantem raio fixo `1.5 km`, exibe KPIs, as 4 imagens censitarias em grade 2x2 (densidade/renda/score/concorrentes) SEM seletor de metrica, tabela de setores e botoes de download CSV/PDF (o download usa `template="classico"`, o default de producao).
