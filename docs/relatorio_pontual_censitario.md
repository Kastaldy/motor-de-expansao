...
Regras implementadas:
- **3 camadas, 1 por mapa** (Densidade populacional, Renda per capita, Concorrentes e Ultra), geradas numa unica chamada e exibidas/embutidas juntas (UI sem dropdown que esconda camadas; PDF com uma pagina por camada). Densidade/Renda compartilham basemap, bbox, projecao, pins e choropleth (varia so o fill dos setores + legenda/titulo); a camada **Concorrentes e Ultra e SO de pins** (sem choropleth de score — BLK-CENSO-03): basemap + pins de concorrentes/Ultra + ponto central + legenda de pins.
...
- `gerar_pdf_relatorio_pontual_censitario(result, mapas=None, *, residual=None, ultra_dir=None)`: retorna bytes PDF em memoria com a **estrutura de 7 paginas** (template Ultra), nesta ordem:
  ...
  3. **Renda** — mapa `renda`.
  4. **Concorrentes e Ultra** — mapa `concorrentes` (mapa SO de pins: basemap + pins de concorrentes/Ultra + ponto central, SEM choropleth de score — BLK-CENSO-03).
