...
| `reguas.taxa_fitness` | Zero leitores do valor do perfil: a taxa efetiva é **calibrada em runtime** (`calcular_colunas_mercado.calibrar_taxa_fitness_mercado`) e o número do perfil seria só o fallback. Entra no BLK-INTL-10. |
| ~~`metas_big_numbers`~~ e ~~`faixas_renda`~~ | **A recusa CAIU — os dois foram promovidos ao schema.** A justificativa original (só lidas por `ponto`/`municipal`, que o Bloco C bloqueia no dia 1) valia enquanto o único leitor era o Relatório Pontual. `metas_big_numbers` voltou em 2026-09-02 (`dashboard/censo_report.py` lê as sete do perfil) e `faixas_renda` em 2026-09-08 (os choropleths de renda do PDF — e, desde o PR #328, o mapa de calor censitário do próprio piloto, que é superfície `mapa`). `reguas.faixas_densidade` nasceu direto no schema em 2026-09-10, pelo mesmo caminho e com a mesma validação. |
