...
#
# `[DEC-065]` **O BLK-MA-20 decidiu: o TotalPass ENTRA, nas duas pontas.** Até 2026-09-16 este
...
#
# `--fontes wellhub` reproduz o universo anterior à DEC-065 sem mexer em código.
...
def resolver_fontes(args: argparse.Namespace) -> tuple[str, ...] | None:
    ...
    `[DEC-065]` **Houve, por algumas horas de 2026-09-16, um par de recortes** (`RecorteFontes`),
...
def _pressao_por_academia(
    ...
    *,
    ...
    pressao = calcular_pressao_por_academia(
        ...
        # `[DEC-065]` A passagem por NOME dentro da MESMA fonte, enfim LIGADA. Ela estava
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if fontes is None:
        ...
        _logger.info(
            ...
            ",".join(sorted(fontes)),
            "" if args.fontes else " (default da DEC-065; use `--fontes wellhub` para o anterior)",
        )
