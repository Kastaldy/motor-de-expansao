...
# a vista aerea (BLK-SAT) soma mais uma quando presente.
PDF_SECTION_HEADERS = (
...
def _classico_socioeconomia_residual_page(
    ...
    *,
    ...
    Socioeconomia = `score_setor_2022_calibrado` por setor censitario no raio de 1,5 km;
    Residual Fitness = `oferta_efetiva_disponivel` (alunos) por hexagono H3 res-7 no raio de
    EXIBICAO de 5 km. Camada ausente -> fallback textual da propria `_draw_maps_grid`
    (offline-safe). READ-ONLY sobre o M1.
    """
    pdf.add_page()
    _draw_full_page_background(pdf, assets.get("conteudo"), ULTRA_BRANCO_GELO)
...
def _classico_competitors_page(
    ...
    *,
    ...
    """Concorrentes: banda classica + mapa CENTRALIZADO (so-pins) + faixa inferior com as
    redes no raio (sem PII). Pedido de Felipe 2026-07-23: centralizar o mapa e remover o
    titulo interno "Concorrentes e Ultra" (redundante com a barra) e a legenda."""
    pdf.add_page()
    _draw_full_page_background(pdf, assets.get("conteudo"), ULTRA_BRANCO_GELO)
...
def gerar_pdf_relatorio_pontual_classico(
    ...
    *,
    ...
    maximo mais 4 -> teto de 11 paginas (era 12 com o entorno); `foto_satelite` (BLK-SAT) soma
    mais uma quando presente.
    ...
    # BLK-RELPON-10: slide-hero "Socioeconomia e Residual Fitness" ANTES dos mapas de calor.
...
def gerar_pdf_relatorio_pontual_censitario(
    ...
    *,
    ...
    A assinatura e' um SUPERSET da anterior: alem dos kwargs historicos, aceita `now`, que so a
    classica aceitava. Qualquer chamada que funcionava antes continua funcionando (o parametro
    novo tem default inerte) — inclusive as do piloto web (`fotos`, `info_imovel`,
    `viabilidade`, `foto_satelite`).
    """
    warnings.warn(
