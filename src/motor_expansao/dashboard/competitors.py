...
# `[DEC-063]` TotalPass. Ate' 2026-09-17 havia UM app so': enquanto o D9 da DEC-039 mantinha o
...
# desenho (`_project_points`). Ate' a DEC-063 a pergunta era "a chave e' vazia?" e `bool()` bastava,
# porque so' havia uma resposta possivel.
# `[DEC-063 / fatia 2]` A academia que esta' nos DOIS apps. Ela NAO tem arte propria: o tile
...
# `[DEC-063 / fatia 2]` A arte do terceiro estado para a WEB.
#
...
def chave_agregador_da_fonte(fonte: object, fontes: object = None) -> str:
    ...
    `[DEC-063 / fatia 2]` `fontes` e' a coluna `fontes_da_academia` (`alvos_ma_nomeados_v8`), que
    ...
    `[DEC-063]` Fonte ausente ou desconhecida cai no WellHub, e isso e' deliberado: o artefato
...
# para as independentes de cada app (DEC-063)
...
def preload_logos(competitors_dir: Path, ultra_dir: Path | None = None) -> None:
    ...
    # `[DEC-063]` Uma passada POR APP: cada um tem arte e cor proprias, e a precedencia
...
def _render_square_logo_tile(
    ...
    *,
    ...
    if key == "__ultra__":
        ...
        # `[DEC-063]` A cor sai do APP: rosa no WellHub, verde no TotalPass. E' ela que carrega a
    ...
    # `[DEC-063 / fatia 2]` O terceiro estado: academia nos DOIS apps.
    #
    ...
    # `[DEC-063 / fatia 2]` A B5: sem arte, o "ambos" vira placa BIPARTIDA (rosa | verde) em vez de
